...
## アピールポイント
- このアプリは、ただの集計アプリではなく「空間演出」ができるアプリである。
- 結果を知ることがメインではなく、結果が出るまでの溜めを作ることによりドキドキ感を演出している。
- 投票結果をグラフで表示することにより、視覚的にわかりやすい結果の取得ができるようにした。
- 管理画面からGUIでお題を変更することができる。
- データベースを使用することにより、スタイリッシュなアプリへと仕上がった。
...
* **投票画面**: 選択された問題の質問文と、巨大な円形ボタン（A/B）を表示。ボタンクリックで投票を実行し、結果画面へ更新。
...
| **Data & Network** | [@Akiraao4532さんのID](https://github.com/Akiraao4532) | データベース設計(SQLite)、過去ログ機能、通信環境設定 |
